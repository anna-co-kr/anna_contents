...
**목적**: 안나가 AI 이미지 생성(Midjourney, Nano Banana Pro) 시 **낮은 프롬프트 활용 능력을 AI가 보좌**해 머릿속 이미지를 프롬프트로 번역하는 gap을 메운다. 레퍼런스 토큰 분해·페어 로그·리믹스로 **프롬프트 품질 향상 → 원하는 이미지 추출 증가 → 수렴 이터레이션 감소 → 포스트당 2시간 병목 해소**의 3단 인과로 연결된다.

**사용자**: 안나 1인 (Instagram/YouTube 글로벌 디자인 큐레이터 채널, 단일 Supabase 계정)

**도구 사용 양상 (실사용 기반, 2026-04-24 확인)**:
...
- **자기진단 약점**: 단순 설명으로 이미지를 마음에 들 때까지 반복 → 시간 소요 큼. 도구의 가장 직접적 가치는 프롬프트 표현력 보조

**성공 기준 (2-tier)**:
- **1차 — 프롬프트 품질 uplift (핵심)**: 페어 저장 시 프롬프트 자체 만족도(1~5 self_rating) 평균이 T1~T3에서 T0 추정 대비 상향. F006 리믹스 도입 시 "AI 제안 채택률"로 객관화
- **2차 — 시간·이터레이션 감축**: T0 추정치(포스트당 2시간, 본 문서 명시) 대비 체감 시간 -60%, 이터레이션 -50%. Task 000(T0 실측)은 시간 제약으로 스킵 확정 → 페어 로그 `iteration_count_cumulative`가 유일한 객관 비교 지표
...
2. 레퍼런스 라이브러리 페이지 (메인 허브)
   ...
   [URL/이미지 드롭] → Claude Vision 토큰 분해(영어 고정) → 레퍼런스 카드 생성
                       ↓
...
5. 토큰 diff 페이지 (V1.5)
   ↓ 성공/실패 프롬프트 토큰 차이 확인 (동일 언어 내 비교)

6. 유사 레퍼런스 검색 페이지 (V1.5)
   ↓ pgvector 유사도 검색 결과 확인

7. 리믹스 제안 페이지 (V1.5)
   ↓ Claude가 기준 레퍼런스의 tool·language에 맞춰 변형 프롬프트 후보 3개 생성
      (MJ 기준 → 영어 후보, NBP 기준 → 한국어 후보)
...
|----|--------|------|--------------|------------|
| **F001** | URL/이미지 드롭 + Vision 토큰 분해 | IG/Pinterest/YouTube/Are.na URL 또는 이미지 파일 드롭 시 Claude Sonnet Vision이 subject/style/lighting/composition/medium/mood 6차원 구조화 토큰으로 자동 분해 (Vision 응답 언어는 **영어 고정** — 토큰은 도구 독립적 의미 단위, 도구별 언어 변환은 F006이 담당) | "이 레퍼런스 같은 결과를 **MJ/NBP에서** 어떻게 뽑는가"의 역공학 반복이 2시간 병목의 근원 — 언어 번역 gap을 Vision이 메움 | 레퍼런스 라이브러리 페이지 |
| **F002** | 좋아요·태그·프롬프트 스니펫 필드 | 레퍼런스에 1~10 점수, 수동 태그(카테고리/무드/색감/용도), 프롬프트 스니펫 저장 및 [copy prompt] 버튼으로 클립보드 복사. 스니펫 저장 시 **tool(midjourney|nano-banana)·language(en|ko)** 메타 필수 | 취향 데이터 축적 없이는 반복 실험이 의미 없음. copy prompt → **MJ(영어) 또는 NBP(한국어) 붙여넣기**가 실질 워크플로우 단축 핵심. 도구·언어 메타가 있어야 F005 유사 검색·F006 리믹스에서 언어 일관성 보장 | 레퍼런스 라이브러리 페이지, 레퍼런스 상세 페이지 |
...
| **F006** | 리믹스 제안 | 저장된 레퍼런스 선택 후 "이 느낌 × 새 주제" 입력 시 Claude가 프롬프트 변형 후보 3개 생성. **기준 레퍼런스의 연결 프롬프트(있을 경우) tool·language에 맞춰 자동 분기 — MJ 기준이면 영어, NBP 기준이면 한국어. 사용자가 명시적으로 도구 전환 선택 시 반대 언어로도 생성 가능**. 대화형 UX, 폼 최소화 | 프롬프트 언어 표현 속도를 Claude가 보조. **"프롬프트 활용능력이 낮다"는 안나의 1차 pain에 가장 직접 대응**. 안나의 평가 속도가 표현 속도를 앞지르는 gap을 메움 | 리믹스 제안 페이지 |
...
| **사용자 행동** | URL 붙여넣기 또는 이미지 파일 드래그 드롭 → Vision 토큰 분해 확인 → 점수/태그/스니펫 입력(tool·language 메타 지정) → [copy prompt]로 MJ(영어) 또는 NBP(한국어)에 붙여넣기 |
| **주요 기능** | - IG/Pinterest/YouTube/Are.na URL 입력 + 이미지 drag-drop 영역<br>- Claude Sonnet Vision 6차원 토큰 자동 분해 표시 (subject/style/lighting/composition/medium/mood, **영어 고정**)<br>- Vision 호출 실패 시 수동 태그 입력 UI 폴백<br>- 레퍼런스 카드 그리드 (썸네일·토큰·점수·태그 요약)<br>- 1~10 점수 입력, 카테고리/무드/색감/용도 태그 추가<br>- 프롬프트 스니펫 텍스트 필드 (여러 버전 저장 가능, 각 스니펫에 **tool(midjourney|nano-banana)·language(en|ko)** 필수 선택)<br>- **[copy prompt]** 버튼 (클립보드 복사, 스니펫의 tool·language가 헤더 배지로 노출)<br>- 레퍼런스 카드 클릭 시 레퍼런스 상세 페이지 이동 |
...
| **사용자 행동** | 기준 레퍼런스 선택 → tool 확인/변경(기본은 레퍼런스의 최근 프롬프트 tool) → 새 주제/주요 변경 사항 입력 → Claude 후보 3개 확인(MJ면 영어, NBP면 한국어) → 마음에 드는 후보 [copy prompt] → MJ/NBP 실행 → 결과를 프롬프트 페어 로그 페이지에 저장 |
| **주요 기능** | - 기준 레퍼런스 선택 (라이브러리에서 검색·선택)<br>- **tool 토글** (Midjourney / Nano Banana Pro, 기준 레퍼런스의 최근 성공 프롬프트 tool을 기본값으로 제안)<br>- "이 느낌 × 새 주제" 자연어 입력 필드 (폼 최소화, 대화형 UX)<br>- Claude Sonnet이 생성한 변형 프롬프트 후보 3개 카드 (**tool에 맞춘 언어로 생성** — MJ=영어, NBP=한국어)<br>- 각 후보별 **[copy prompt]** 버튼 (생성된 prompts row에 tool·language·source=`remix` 자동 기록)<br>- 후보 재생성 버튼 |
| **다음 이동** | [copy prompt] → 클립보드 복사 후 프롬프트 페어 로그 페이지로 이동 유도 (tool·language prefill) |
