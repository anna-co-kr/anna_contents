...
- **자동 6차원 분석 셋업** (2026-04-25 Task 016-3): 안나가 라이브러리에 URL/이미지 드롭만 하면 분석은 Claude Code(=이 CLI)가 담당. 흐름: `references` 테이블에 `reference_tokens`가 없는 카드를 SQL로 조회 → `scripts/sign-thumbnail.ts`로 1h signed URL 발급 → 이미지 다운로드/Read → 6차원 영어 JSON 생성 → Supabase MCP로 INSERT(`source='claude-code'`). **수동 호출**: `/auto-analyze` slash command 또는 자연어("분석 대기 카드 처리해줘"). **자동 호출**: cron 매일 12:03 / 18:33 KST (현재 session-only — 세션 종료 시 사라짐, 매 세션 시작 시 재등록 필요. 향후 macOS launchd + `claude -p` headless 전환 검토). 결과는 `docs/auto-analysis-log.md`에 누적. **운영비 0원 유지** (Anthropic·Voyage·Gemini 등 외부 API 호출 없음, 안나의 Claude Code MAX 자원만 사용).
